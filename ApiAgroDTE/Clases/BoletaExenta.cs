...
namespace ApiAgroDTE.Clases
{
    public class BoletaExenta
    {
        ...
        public string crearBoleta(JsonElement Encabezado, JsonElement Detalles, JsonElement ReferenciaOp, Boolean RefFlag, JsonElement DscRcgGlobalOp, Boolean DscRcgFlag, int nuevoFolio, string archxml, string T41B)
        {
            ...
            string MntNetoOp = "";//Opcional
            if (Totales.TryGetProperty("MntNeto", out var MntNeto))
            {
                MntNetoOp = MntNeto.ToString();
            }
            ...
            string TotalPeriodoOp = "";//Opcional
            if (Totales.TryGetProperty("TotalPeriodo", out var TotalPeriodo))
            {
                TotalPeriodoOp = TotalPeriodo.ToString();
            }
            ...
            writer.WriteAttributeString("ID", T41B);
            ...
            writer.WriteElementString("MontoNF", "0");

            if (!string.IsNullOrEmpty(TotalPeriodoOp))
            {
                writer.WriteElementString("TotalPeriodo", TotalPeriodoOp);
            }
            ...
            for (int i = 0; i < detallesArray.Count(); i++)
            {
                ...
                var IndExeStr = detalleObject.IndExe.ToString();
                ...
                writer.WriteElementString("IndExe", IndExeStr);
            ...
            string respuestaFinal = certificado.firmarConCertificado(T41B, oDocument, archxml);
            ...
            string queryUpdateInicial = "UPDATE boleta_exenta SET fechaemis_boleta_exenta = '" + FchEmis + "',mnttotal_boleta_exenta = '" + MntTotal + "',detalle_boleta_exenta = '" + DetalleFactura + "'";
            string queryUpdateRef = ",folioref_boleta_exenta = '" + FolioRef + "', tipo_dteref_boleta_exenta ='"+TpoDocRef+"'";
            string queryUpdateFinal = " WHERE folio_boleta_exenta = '" + Folio + "'";
            string queryUpdate = "";
