...
*/
           ...
           //REFERENCIA OPCIONAL----------------------------------<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
             ...
             if (CodRef == "1")
            {
                ...
                string respuestaND1 = operaciones.VerificarND1(MntNeto,TasaIVA,IVA,MntTotal, FolioRef);

                if(respuestaND1 == "ta bien"){
                }else{
                    //ELIMINAR EL ARCHIVO DE LA FACTURA
                    writer.WriteEndDocument();
                    writer.Flush();
        ...
        //-----------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            string queryUpdate = "UPDATE nota_debito SET fchemis_nota_debito = '" + FchEmis + "',rutrecep_nota_debito = '" + RUTRecep + "',rznsocrecep_nota_debito = '" + RznSocRecep + "',cmnarecep_nota_debito = '" + CmnaRecep + "',mnttotal_nota_debito = '" + MntTotal + "',detalle_nota_debito = '" + DetalleFactura + "',folioref_nota_debito = '" + FolioRef + "' , tipo_dteref_nota_debito ='"+TpoDocRef+"' WHERE folio_nota_debito = '" + Folio + "'";
